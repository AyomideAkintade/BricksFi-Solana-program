...
use crate::{state::AssetState, utils};
...
pub fn initialize_asset(
    ...
    asset_state.key = ctx.accounts.key();
    ...
    let time = utils::current_timestamp();

    asset_state.created_at = created_at;
    asset_state.updated_at = updated_at;

    msg!("Initialized asset {:?} with {}", name, ctx.accounts.key());
